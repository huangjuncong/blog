...
代码结构比上一讲的 IoC 复杂不少。我们首先对包每个包都干啥了什么做一个简单介绍。

* `invocation` 描述的就是一个方法的调用。注意这里是名词，而不是动词。
* `interceptor` 大家最熟悉的拦截器，打结器拦截的目标就是 `invcation` 包里面的调用。
...
这个时候宏观的看我们大概梳理出了一条路线， `adaper` 将 `advisor` 适配为 `interceptor` 去拦截 `invoctoin`。

下面我们从这个链条的最末端讲起：

### `invcation`

首先看 `MethodInvocation` 作为所有方法调用的接口。要描述一个方法的调用包含三个方法，获取方法本身`getMethod`,获取方法的参数`getArguments`，最后执行方法本身`proceed()`。
...
`ProxyMethodInvocation` 看名字，作为一个代理方法的调用，增加了一个获取代理的方法。
```java
...
`AopMethodInterceptor` 是 Aop 容器所有拦截器都要实现的接口调用方法：
```java
...
看了上面的代码我们发现，实际上 `mi.proceed()`才是执行原有的方法。而`advice`我们上文就说过，是描述增强的方法”干什么“的数据结构，所以对于这个before拦截器，我们就把advice放在了真正执行的方法前面。而after拦截器，就放在了真正执行的方法后面。就是那么简单。
...
```

在实际的运用中，我们的方法很可能被多个方法的拦截器所增强。所以我们，使用了一个list来保存所有的拦截器。所以我们需要递归的去增加拦截器。当处理完了所有的拦截器之后，会调用原来的方法。我们可以理解，动态的织入代码就发生在这里。
...
```
`CglibMethodInvocation` 只是重写了 `invokeOriginal` 方法。使用代理类来调用原方法。
...
```
这个就是我们代理对象生成的核心方法。使用 cglib 生成代理类。我们可以与之前ioc框架的代码。比较发现区别就在于：
```java
    Callback callbacks = getCallBack(advised);
    enhancer.setCallback(callbacks);
```
callback与之前不同了，我们就来看看 getCallback 里面的 `DynamicAdvisedIcnterceptor`到底干了啥。

篇幅问题，这里不会介绍 cglib 的使用，已经callback的作用。不理解的同学需要自行学习。
...
``` 
看到这个类我们发现代码实现的 MethodInterceptor 是 gclib的接口，并非我们之前的 AopMethodInterceptor。最终我们看到：
```java
MethodInvocation invocation = new CglibMethodInvocation(obj,targetSource.getTagetObject(),method, args,interceptorList,proxy);
```
这行代码，我们的整个逻辑终于连起来了。也就是这个动态的拦截器，把我们通过 `CglibMethodInvocation` 织入了增强代码的类，委托给了 cglib 生成代理对象。至此我们的 Aop 的核心功能就实现了。
...
```
继承了上一讲我们实现的 IoC 容器的BeanFactoryImpl。重写了 getBean，如果是一个切面代理类，我们使用Aop框架生成代理类，如果是普通的对象，我们就用 IoC 容器注入。
...
Spring 的两大核心特性 IoC 与 Aop 两大特性，希望大家能够深入理解两个特性。

Spring的源码实在是特别复杂，常常给人极大的挫败感，但是我还是坚持下来了，而且有了极大的收获。
